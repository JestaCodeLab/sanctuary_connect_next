...
function TithesPageContent() {
  ...
  // Filter to only tithes
  const donations = allDonations.filter((d: Donation) => d.donationType === 'tithe');
  ...
  const stats = [
    { label: 'Total Tithes', value: totalDonations.toLocaleString(), icon: DollarSign },
    { label: 'Total Amount', value: formatCurrency(totalAmount), icon: Wallet },
    { label: 'Average Tithe', value: formatCurrency(averageDonation), icon: TrendingUp },
    ...
    <div>
      ...
      <Card padding="none">
        ...
        {isLoading ? (
          ...
          <EmptyState
            title="No Tithes Yet"
            description="Start tracking tithes by recording your first tithe."
            icon={DollarSign}
            actionLabel="Record Tithe"
            onAction={() => setIsModalOpen(true)}
          />
