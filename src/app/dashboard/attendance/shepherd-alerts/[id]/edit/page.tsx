...
export default function EditShepherdAlertPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setFormData({
          name: alertData.name || '',
          // branchId comes back populated ({_id, name}) the same way
          // shepherds.memberId does — flatten to a plain ID string.
          branchId: typeof alertData.branchId === 'string' ? alertData.branchId : alertData.branchId?._id || '',
          // The GET response comes back with shepherds.memberId populated
          // (a full {_id, firstName, lastName, phone} object) so the list
          // below can render names — flatten it back to a plain ID string
          // here, otherwise it round-trips as an object on submit and the
          // server's Member.find({_id: {$in: ...}}) can't match anything.
          shepherds: (alertData.shepherds || []).map((s: any) => ({
            memberId: typeof s.memberId === 'string' ? s.memberId : s.memberId?._id,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/attendance/shepherd-alerts/${alertId}`);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex items-center gap-3">
        <Link href={`/dashboard/attendance/shepherd-alerts/${alertId}`}>
          <button className="p-2 hover:bg-accent rounded-lg transition-colors">
            <ArrowLeft className="w-5 h-5" />
          </button>
        </Link>
