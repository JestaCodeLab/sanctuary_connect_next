...
import { Zap, CreditCard, Check, AlertCircle, Loader2, Package } from 'lucide-react';
...
export default function CreditsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Current Balance */}
      <Card padding="lg" className="bg-gradient-to-br from-primary/10 to-primary/5 border-primary/20">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-3xl font-bold text-primary">{(creditsBalance?.balance ?? 0).toLocaleString()}</p>
              <p className="text-xs text-muted mt-0.5">
                {creditsBalance?.balance === 1 ? 'credit available' : 'credits available'}
              </p>
            </div>
          </div>
          <CreditCard className="w-10 h-10 text-primary/20 flex-shrink-0 hidden sm:block" />
        </div>
      </Card>
            ...
            {sortedPackages.map((pkg) => (
              <Card
                ...
                className={`relative flex flex-col transition-all hover:shadow-md ${
                  pkg.isPopular ? 'border-2 border-primary' : 'border-2 border-border hover:border-primary/50'
                }`}
              >
                {pkg.isPopular && (
                  <div className="absolute -top-2.5 right-4 px-2.5 py-0.5 bg-primary text-white text-xs font-bold rounded-full shadow-sm">
                    Popular
                  </div>
                )}
